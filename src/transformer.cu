// transformer.cu — Single decoder layer forward pass (Qwen2.5 architecture)
...
TransformerLayer::TransformerLayer(const TransformerConfig& cfg,
                                   CublasHandle& cublas)
    : cfg_(cfg), cublas_(cublas) {}

// ---- cuBLAS linear: out[M,N] = in[M,K] @ W[N,K]^T ----
void TransformerLayer::linear(half* out, const half* in,
                               const half* W, int M, int N, int K) {
    half alpha = __float2half(1.0f);
    half beta  = __float2half(0.0f);
    // Row-major → cuBLAS column-major trick:
    // C_col[N,M] = W_col^T[N,K] @ X_col[K,M]
    CUBLAS_CHECK(cublasHgemm(cublas_.get(),
        CUBLAS_OP_T, CUBLAS_OP_N,
        N, M, K, &alpha, W, K, in, K, &beta, out, N));
}
...
GpuTensor TransformerLayer::attention(const GpuTensor& x,
    ...
    {
        half alpha = __float2half(1.0f / sqrtf(static_cast<float>(D)));
        half beta  = __float2half(0.0f);
        // Batched: for each head h,  scores[h] = Qt[h] @ Kt[h]^T
        CUBLAS_CHECK(cublasHgemmStridedBatched(cublas_.get(),
            CUBLAS_OP_T, CUBLAS_OP_N,
            S, S, D,
            &alpha,
            Kt.data(), D, S * D,     // Kt[h]: [S,D] col-major=[D,S], trans→[S,D]
            Qt.data(), D, S * D,     // Qt[h]: [S,D] col-major=[D,S]
            &beta,
            scores.data(), S, S * S,
            nh));
    }
